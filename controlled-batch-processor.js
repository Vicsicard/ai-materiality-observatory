// Controlled batch processor for 10 candidates
export default {
  async fetch(request, env) {
    const url = new URL(request.url);
    
    if (url.pathname === '/controlled-batch-process') {
      try {
        console.log('=== CONTROLLED BATCH PROCESSING - 10 CANDIDATES ===');
        ...
        console.log('Database Counts Before:', beforeCounts);
        
        // Step 1: Select exactly 10 approved candidates
        ...
        console.log(`Step 1 - Selected ${totalSelected} approved candidates`);
        
        if (totalSelected === 0) {
          return new Response(JSON.stringify({
            success: true,
            message: 'No approved candidates found for processing',
            batch_report: {
              total_selected: 0,
              total_processed: 0,
              total_failed: 0,
        ...
        for (const candidate of candidates.results) {
          ...
          try {
            // Step 2a: Execute MaterialitySignalPipeline (mock for this test)
            const pipelineInput = {
            ...
            // Mock pipeline output (in production, this would be real MaterialitySignalPipeline)
            ...
            // Step 2e: Update Candidate Status
        ...
        // Step 3: Get database counts after processing
        ...
        console.log('Database Counts After:', afterCounts);
        ...
        const batchReport = {
          ...
          total_articles_created: totalArticlesCreated,
    ...
    return new Response('Controlled batch processor - use /controlled-batch-process');
  }
};
